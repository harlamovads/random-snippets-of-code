...
if searchmode == "2" or searchmode == "3":
    nlp = spacy.load("en_core_web_sm")

dict1 = {} #словарь с номерами кусков текста
...
for i in tqdm(range(len(textlist))):
    ...
    if searchmode == "2" or searchmode == "3":
        ...
        if searchmode == "3": # для режима 3: собираем список со словарями частотности для каждого куска текста
            ...
            dict3 = {}
    if searchmode == "1": # для режима 1: забиваем на спейси и просто присоединяем те имена из worklist_1, которые есть в абзаце. Довольно лениво.
        res = []
        for ima in worklist_1:
            if ima in abz:
                res.append(ima)
            dict1[count] = res

if searchmode == "3": # для режима 3: для каждого словаря из chapterlist ищем топ-3 самых частотных имени собственных. 
    for dh in range(len(chapterlist)):
        if len(chapterlist[dh]) > 3:
            for s in range (3):
